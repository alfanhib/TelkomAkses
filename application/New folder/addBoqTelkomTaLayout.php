<html>

<head>
    <title>BOQ Telkom TA</title>
    <link rel="stylesheet" href="style/bootstrap/css/bootstrap.min.css">
    <link rel="stylesheet" href="style/bootstrap-switch/dist/css/bootstrap3/bootstrap-switch.min.css">
    <link rel="stylesheet" href="style/fontawesome/css/font-awesome.min.css">
    <link rel="stylesheet" href="style/headStyle.css">
    <link rel="stylesheet" href="style/contStyle.css">
    <link rel="stylesheet" href="style/addBoqStyle.css">
    <link rel="stylesheet" href="style/footStyle.css">


    <script src="script/jquery.min.js"></script>
    <script src="script/myScript.js"></script>
    <script src="style/bootstrap/js/bootstrap.min.js"></script>
    <script src="style/bootstrap-switch/dist/js/bootstrap-switch.min.js"></script>
</head>

<body>
    <nav class="head navbar navbar-toggleable-md navbar-light bg-faded sticky-top">
        <button class="navbar-toggler navbar-toggler-right" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
        </button>
        <a class="navbar-brand" href="#" style="color: white;">
                <img src="assets/Logo2.jpg" width="30" height="30" class="d-inline-block align-top" alt="">
                TelkomAkses - Solo
        </a>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                <li class="nav-item">
                    <a class="nav-link" href="#" style="color: white;">Dashboard</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#" style="color: white;">Harga Satuan</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="color: white;">
                          BOQ
                    </a>
                    <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                        <a class="dropdown-item" href="#">BOQ List</a>
                        <a class="dropdown-item" href="#">BOQ Telkom-TA</a>
                        <a class="dropdown-item" href="#">BOQ TA-Mitra</a>
                    </div>
                </li>
            </ul>
            <button class="btn btn-danger">Logout <i class="fa fa-sign-out"></i></button>
        </div>
    </nav>
    <div class="cont col-md-12">
        <div class="main col-md-12">
            <div class="main-head">
                <div class="row">
                    <div class="input-group inputData col-md-4">
                        <input type="text" class="form-control" placeholder="Search Designator">
                        <div class="input-group-btn">
                            <button class="btn btn-default" type="submit">
                                    <i class="fa fa-search"></i>
                                </button>
                        </div>
                    </div>
                    <div class="col-md-3 inputData">
                        <input type="text" class="form-control" placeholder="File Name">
                    </div>
                    <div class="col-md-3">
                        <div class="btn-group">
                            <button class="btn btn-primary"><i class="fa fa-download"></i> Download</button>
                            <button class="btn btn-success"><i class="fa fa-upload"></i> Upload</button>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <h4>BOQ Telkom TA</h4>
                    </div>
                </div>
            </div>
            <div class="main-cont" style="">
                <table id="tableBoq" class="table table-hover table-bordered table-stripped" style="width: 1650px;">
                    <thead>
                        <tr>
                            <th rowspan="2" style="text-align: center; vertical-align: middle;">No</th>
                            <th rowspan="2" style="width: 250px; text-align: center; vertical-align: middle;">Designator</th>
                            <th rowspan="2" style="width: 700px; text-align: center; vertical-align: middle;">Uraian</th>
                            <th rowspan="2" style="text-align: center; vertical-align: middle;">Satuan</th>
                            <th colspan="2" style="text-align: center; vertical-align: middle;">Volume</th>
                            <th colspan="2" style="text-align: center; vertical-align: middle;">Paket 5</th>
                            <th colspan="2" style="text-align: center; vertical-align: middle;">Telkom</th>
                            <th rowspan="2" style="text-align: center; vertical-align: middle;">Action</th>
                        </tr>
                        <tr>
                            <th style="width: 150px; text-align: center; vertical-align: middle;">PLAN</th>
                            <th style="width: 150px; text-align: center; vertical-align: middle;">AANW</th>
                            <th style="width: 150px; text-align: center; vertical-align: middle;">Material</th>
                            <th style="width: 150px; text-align: center; vertical-align: middle;">Jasa</th>
                            <th style="width: 150px; text-align: center; vertical-align: middle;">Material</th>
                            <th style="width: 150px; text-align: center; vertical-align: middle;">Jasa</th>
                        </tr>
                        <tr>
                            <td colspan="11">A. Kabel, Penyambungan dan Pekerjaan Sipil</td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="text-align: center;">1</td>
                            <td>DC-OF-SM-12D</td>
                            <td>DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">
                                <button class="btn btn-danger"><i class="fa fa-trash"></i></button>
                            </td>
                        </tr>
                    </tbody>
                    <thead>
                        <tr>
                            <td colspan="11">B. Manhole, Perbaikan dan Modifikasi</td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="text-align: center;">1</td>
                            <td>DC-OF-SM-12D</td>
                            <td>DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">
                                <button class="btn btn-danger"><i class="fa fa-trash"></i></button>
                            </td>
                        </tr>
                    </tbody>
                    <thead>
                        <tr>
                            <td colspan="11">C. New Item</td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="text-align: center;">1</td>
                            <td>DC-OF-SM-12D</td>
                            <td>DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">
                                <button class="btn btn-danger"><i class="fa fa-trash"></i></button>
                            </td>
                        </tr>
                        <tr>
                            <td style="text-align: center;">1</td>
                            <td>DC-OF-SM-12D</td>
                            <td>DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">DC-OF-SM-12D</td>
                            <td style="text-align: center; vertical-align: middle">
                                <button class="btn btn-danger"><i class="fa fa-trash"></i></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <div class="main-foot">

            </div>
        </div>
    </div>
    <div class="foot col-md-12">

    </div>
</body>

</html>
